...
static DM::TDV GetRadiusFromSample(const vector<CSampleIDWeight> &sample, const TDV &center, const TDM &scale); 

static bool GetCenterScaleFromSample(const vector<CSampleIDWeight> &sample, TDV &center, TDM &scale, double t, int posterior_type=POSTERIOR_HEATED); 
...
static TDV GetRadiusFromSample(const vector<CSampleIDWeight> &sample, const TDV &center, const TDM &scale)
{
	TDV Radii(sample.size(), 0.0); 
...
static bool GetCenterScaleFromSample(const vector<CSampleIDWeight> &sample, TDV &center, TDM &scale, double lambda, int posterior_type)
{
	if (sample.empty())
	{
	...
	TDV sample_sum(sample[0].data.Dim(), 0.0);
	TDM sample_square(sample[0].data.Dim(), sample[0].data.Dim(), true, 0.0); 
	for (int i=0; i<(int)sample.size(); i++)
	{
...
{	
	...
	TMatrix scale_matrix = CreateMatrix(scale.Rows(), scale.Cols()); 
	TVector R_vector = CreateVector(R.Dim()); 
	for (unsigned int i=0; i<center.Dim(); i++)

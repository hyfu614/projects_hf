...
int CSampleIDWeight::GetSize_Data() const
{
	...
	return sizeof(int)+(int)data.Dim()*sizeof(double)+sizeof(int)+sizeof(double)+sizeof(reserved)+sizeof(bool);	// changed by HF
	// data.dim, data.vector, id, weight, reserved, calculated
}

CSampleIDWeight::CSampleIDWeight(const TDV &_x, int _id, double _weight, double _reserved, bool _calculated) : id(_id), weight(_weight), reserved(_reserved), calculated(_calculated)
{
	//data.CopyContent(_x);
	data = _x;	// changed by HF 
}
...
bool CSampleIDWeight::PartialCopyFrom(const CSampleIDWeight &right, int offset, unsigned int length)
{
	//if (data.dim < offset+(int)length || right.data.dim < offset+(int)length)
	if (data.Dim() < offset+length || right.data.Dim() < offset+length)	// changed by HF
	{
...
bool CSampleIDWeight::PartialCopyFrom(int offset1, const CSampleIDWeight &right, int offset2, unsigned int length)
{
	//if (data.dim < offset1+(int)length || right.data.dim < offset2+(int)length )
	if (data.Dim() < offset1+length || right.data.Dim() < offset2+length )	// changed by HF
	{
		cerr << "CSampleIDWeight::PartialCopyFrom() : index exceeding dimension" << endl; 
		return false; 
	}
...
ostream& write(ostream & output_stream, const CSampleIDWeight *x)
{
	...
	unsigned int dim=x->data.Dim();
        output_stream.write((char*)&dim, sizeof(int));	// changed by HF 
	double temp_x; 
...
istream& operator >> (istream &inputStr, CSampleIDWeight &sample)
{
	...
	for (unsigned int i=0; i<sample.data.Dim(); i++)
		inputStr >> sample.data[i];	// changed by HF 
	inputStr >> sample.id; 

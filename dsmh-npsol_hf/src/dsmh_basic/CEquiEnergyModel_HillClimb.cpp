...
std::vector<CSampleIDWeight> CEquiEnergyModel::HillClimb_NPSOL(int nSolution, int optimization_iteration, int perturbation_iteration, double perturbation_scale, double scale, const DM::TDV &start_point)
{
	...
	int n = current_sample.data.Dim();	// current_sample.data.dim;	sample dimension, changed by HF
	// linear constraints
	...
	DM::TDV peak(n,0.0), perturbation(n,0.0), x_plus_perturbation(n,0.0);  
	DM::TDM hessian_cholesky(n,n,0.0); 
	std::vector<CSampleIDWeight> solution; 

	try {
		for (int i=0; i<nSolution; i++)
		{
			if (start_point.Dim())		// (start_point.dim) changed by HF
				//memcpy(x, start_point.vector, sizeof(double)*start_point.dim); 
				memcpy(x, start_point.Vector(), sizeof(double)*start_point.Dim());	// chnaged by HF
			else 
			{
			...
			for (int ii=0; ii<optimization_iteration; ii++)
			{
				...
				for (int jj=0; jj<perturbation_iteration; jj++)
				{
					...
					memcpy(x_plus_perturbation.Vector(), x, sizeof(double)*n);	// changed by HF 	
				...
				if (ii==0 || -f > log_posterior_optimal)
				{
					...
					memcpy(peak.Vector(), x, sizeof(double)*n);
                        		memcpy(hessian_cholesky.Matrix(), R, sizeof(double)*n*n); 
                        		hessian_cholesky.ForceColumnMajor();		// changed by HF
				}
			}
			...
			if (log_posterior_optimal > MINUS_INFINITY)
			{
				nAccpt ++; 
				// Mean 
				//DM::TDV copy(peak.dim); 
				//copy.CopyContent(peak); 
				//gmm_mean.push_back(copy);  
				DM::TDV copy(peak); 
				gmm_mean.push_back(copy);	// changed by HF 
				// Hessian = inverse of covariance matrix
				//DM::TDM hessian = Transpose(hessian_cholesky)*hessian_cholesky; 
				//hessian = 0.5*(hessian+Transpose(hessian)); 
				DM::TDM hessian = T(hessian_cholesky) * hessian_cholesky; 
				hessian = 0.5*(hessian+T(hessian));		// changed by HF
				DM::TDM eVectorHessian; 
				DM::TDV eValueHessian; 
				...
				//DM::TDV diffusedEValue(eValueHessian.dim,0.0), diffusedEValueInverse(eValueHessian.dim,0.0); 
				DM::TDV diffusedEValue(eValueHessian.Dim(),0.0), diffusedEValueInverse(eValueHessian.Dim(),0.0); 
				...
				//}
				for (unsigned int i=0; i<eValueHessian.Dim(); i++)	// changed by HF
				{
					diffusedEValue(i) = sqrt(scale/eValueHessian(i)); 
					diffusedEValueInverse(i) = sqrt(eValueHessian(i)/scale); 
					covariance_sqrt_log_determinant += -0.5*log(eValueHessian(i)/scale); 
				...
				gmm_covariance_sqrt.push_back(eVectorHessian*Diag(diffusedEValue)*T(eVectorHessian)); 
				gmm_covariance_sqrt_inverse.push_back(eVectorHessian*Diag(diffusedEValueInverse)*T(eVectorHessian)); 

...
protected:
	vector<DM::I> block_scheme; 	// assignments of the dimensions into the blocks. block_scheme[i] contains the dimensions assgined to the i-th block, which could be continuous or discontinues (general case)
	vector<DM::TDM> blocks; 	// directions of the block. blocks[i] is n-by-bi,  where n is the dimension of the sample, bi is the size of the i-th block. 
...
public:
	...
	void SetBlocks(const vector<DM::TDM> &_blocks); 
	void SetBlockScheme(const vector<DM::I> &_block_scheme); 

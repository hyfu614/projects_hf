...
private:
	...
	std::vector<DM::TDV> gmm_mean; 
	std::vector<DM::TDM> gmm_covariance_sqrt; 
	std::vector<double> gmm_covariance_sqrt_log_determinant; 
	std::vector<DM::TDM> gmm_covariance_sqrt_inverse; 
...
public:
	...
	bool JumpAcrossStriation(const CSampleIDWeight &y_end, const CSampleIDWeight &y_initial, DM::TDM &) const; 
...
public:
	...
	std::vector<int> Simulation_Within(DM::TDM &jump_table, bool if_storage, const string &sample_file_name=string()); 	// Simulation within the same energy stage (no jumping across stages). Returns the maximum posterior during simulation
	std::vector<int> Simulation_Cross(DM::TDM &jump_table, bool if_storage, const string &sample_file_name=string()); 	// Simulation across stages. Returns the maximum posterior during simulation 	
...
// HillClimb
public:
	std::vector<CSampleIDWeight> HillClimb_NPSOL(int nSolution, int optimization_iteration=1, int perturbation_iteration=0, double perturbation_scale= 1.0, double diffusion_scale= 1.0, const DM::TDV &start_point=DM::TDV(0));
